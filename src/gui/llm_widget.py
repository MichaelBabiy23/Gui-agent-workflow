"""ModelSelector and provider icon helpers — shared by PropertiesPanel."""
...
class ModelSelector(QWidget):
    """Compact model selector with dropdown overlay on a parent widget."""
    ...
    def _select_item(self, item: QListWidgetItem):
        ...
        self._update_button_label()
        if old_id != self._current_model_id:
            self.model_changed.emit(old_id or "", self._current_model_id or "")
...
# ---------------------------------------------------------------------------
# Provider icon helpers — used by PropertiesPanel to populate ModelSelector
# ---------------------------------------------------------------------------

def get_registered_providers():
    """Return all registered LLM providers, lazy-importing if needed."""
...
def provider_company(provider_name: str) -> str:
    return {
        "claude": "Anthropic",
        "codex": "OpenAI",
        "grok": "xAI",
...
def provider_icon(provider_name: str) -> QIcon:
    ...
    if logo_filename:
        ...
        if logo_path.exists():
            ...
            if not source.isNull():
                normalized = _normalized_logo_pixmap(source)
                icon = QIcon(normalized)
                PROVIDER_ICON_CACHE[provider_name] = icon
                return icon

    icon = _fallback_provider_icon(provider_name)
    PROVIDER_ICON_CACHE[provider_name] = icon
    return icon
...
def _fallback_provider_icon(provider_name: str) -> QIcon:
    ...
    if provider_name == "codex":
        ...
        label = provider_company(provider_name)[:1].upper()

...
## Files
- `llm_form.py`: `_LLMForm` — the LLM-call editor. A shared `ChatHeader` (session crumb, node title, live pill, Settings/Output toggle) sits above a two-page stack. The Settings page (its own scroll area) owns the two-stage model selection (Model dropdown plus a dependent Effort dropdown that only appears for models with variants), the account-profile dropdown, the session controls (resume/save/restart/named-resume), per-node prompt-template `Prepend`/`Append` dropdowns, the Prompt editor, the Prompt Preview, and the variable-warning note. The Output page is a `ConversationTabs` from `src/gui/llm_chat/` bound to the node's `ChatConversations`: one tab per real CLI chat, each showing its own `ChatView`.
...
- Non-LLM forms expose `show_output(visible)` and small `set_*`/`current_*` state methods rather than reaching into child widgets from outside. `_LLMForm` exposes `bind_conversations(conversations, title=, session_label=, shared=)`, `set_view`/`view` (`settings`/`output`), `set_header_title`, `set_text_scale`, and `chat_widgets()` (the header and the tabs, which the panel zoom must leave alone). The form switches to Output when a live turn starts in any of the node's chats, and the header `Working` pill reflects whether any chat is busy.
...
- Under the Model dropdown, `cli_status_label` plus `rescan_cli_button` sit in a row that is visible only while some provider CLI is missing. `rescan_installed_clis()` re-detects CLIs, repopulates the dropdown, and re-applies the last id passed to `set_model_state` so the node's stored model survives the rebuild.
